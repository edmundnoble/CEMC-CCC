...
public class Main {
	...
	private static void sumacSequence() {
		...
		println("t2 = ");
		int t2 = nextInt();
		printfln("Sumac length equals %s", nextSumac(t1, t2, -1));
	}

	private static int nextSumac(int t1, int t2, int len) {
		if (len < 0) {
			return nextSumac(t1, t2, 2);
		}
		if (t1 < t2) {
			return len;
		} else {
			return nextSumac(t2, t1 - t2, ++len);
		}
	}
	...
	private static void windAltitude() {
		...
		int firstHour = -1;
		for (int t = 1; t <= M; t++) {
			int a = (int) ((-6 * (Math.pow(t, 4))) + (30 * (Math.pow(t, 3)))
					+ (2 * (Math.pow(t, 2))) + t);
			if (a <= 0) {
				firstHour = t;
				break;
	...
	private static void coinSolve() {
		 ...
		 * She has particular rules that she follows: • The initial coin line-up
		 * defines all positions where coins can be placed. That is, no
		 ...
		 * still exists. • The game consists of a sequence of moves and in each
		 * move Jo moves a coin from one position to an adjacent position. • The
		 * coins can be stacked, and in a move Jo always takes the top coin from
		 * one stack and moves it to the top of another stack. • In a stack of
		 * coins, Jo never places a higher-value coin on top of a lower-value
	...
	private static void shiftWord() {
		 ...
		 * gives B. Sheldon and Leonard’s code depends on a parameter K and also
		 * varies depending on the position of each letter in the word. For the
		 * letter at position P , they use the shift value of S = 3P + K. For
		 * example, here is how ZOOM is encoded when K = 3. The first letter Z
		 * has a shift value of S = 3 × 1 + 3 = 6; it wraps around and becomes
		 * the letter F. The second letter, O, has S = 3 × 2 + 3 = 9 and becomes
		 * X. The last two letters become A and B. So Sheldon sends Leonard the
		 * secret message: FXAB Write a program for Leonard that will decode
		 * messages sent by Sheldon. Input Specification The input will be two
		 * lines. The first line will contain the positive integer K (K < 10),
		 * which is used to compute the shift value. The second line of input
		 * will be the word, which will be a sequence of uppercase characters of
		 * length at most 20. Output Specification The output will be the
		 * decoded word of uppercase letters.
		 */
		// S = 3P + Ks
		...
		for (int i = 0; i < message.length(); i++) {
			int s = (3 * (i + 1)) + k;
			output += (char) lastCharByInt(message.charAt(i), s);
		}
		println(output);
	}
